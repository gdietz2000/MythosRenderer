...
ModelMesh* ObjectLoader::CreateMeshFromOBJ(const char* filepath)
{
	...
	for (int i = 0; i < vertexIndices.size(); ++i) {
		...
		Math::Vector3 vertex = tempVertices[vertexIndex - 1];
		outVertices.push_back(vertex);
	}

	for (int i = 0; i < uvIndices.size(); ++i) {
		unsigned int uvIndex = uvIndices[i];
		Math::Vector2 uv = tempUvs[uvIndex - 1];
		outUvs.push_back(uv);
	}

	for (int i = 0; i < vertexIndices.size(); ++i) {
		ModelVertexInformation data;
		data.m_Vertex = outVertices[i];
		data.m_Uvs = outUvs[i];
